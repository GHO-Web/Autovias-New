class AppCardLeftDescription extends HTMLElement {
  ...
  async _loadAndRenderCards(jsonPath) {
    ...
    try {
      ...
      this.innerHTML =
        '<div class="card-left-container"><p>Error loading special services.</p></div>';
    }
  }

  _render(cardsData) {
    this.innerHTML = `<div class="card-left-container">${cardsData.map((card) => this._getCardHTML(card)).join("")}</div>`;
  }
  ...
  _getCardHTML(card) {
    ...
    if (buttonText && buttonLink) {
      buttonHtml = `
        <a href="${buttonLink}" class="btn-card">
          ${buttonIcon ? `<img src="${buttonIcon}" alt="Button icon" class="btn-icon">` : ""}
          ${buttonText}
        </a>
      `;
    }
