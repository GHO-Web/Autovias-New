class AppBlog extends HTMLElement {
	...
	render() {
		// Si no hay posts, muestra un mensaje y no intentes renderizar el componente.
		if (!this.posts || this.posts.length === 0) {
			this.innerHTML = "<p>No hay blogs para mostrar en este momento.</p>";
			return;
		}
      ...
      <div class="last-notes__items">
        <article class="last-notes__item">
          <a href="${firstPost.link || "#"}" title="${
			firstPost.title?.rendered || "Lorem"
              ...
              <div class="last-notes__card last-notes__card--featured">
                ...
                <div class="last-notes__card__caption">
                  <p class="last-notes__card__caption-date">${
										this.formatDate(firstPost.date) || "13 Mar 2025"
									}</p>
                  ...
                  <p class="last-notes__card__caption-description">
                    ${
											this.limitExcerpt(firstPost.excerpt?.rendered) ||
        ...
        <article class="last-notes__item">
          <a href="${secondPost.link || "#"}" title="${
			secondPost.title?.rendered || "Lorem"
		}" target="_blank" rel="noopener noreferrer" aria-label="Ver más" aria-current="page">
            <div class="last-notes__secondary">
              <div class="last-notes__card last-notes__card--small">
                <div
                  ...
                  style="
                    background-image: url('${
											secondPost._embedded?.["wp:featuredmedia"]?.[0]
												?.source_url || ""
										}');
                  "
                ></div>
                <div class="last-notes__card__caption">
                  <p class="last-notes__card__caption-date">${
										this.formatDate(secondPost.date) || "28 Feb 2025"
									}</p>
                  <h3 class="last-notes__card__caption-title">
                    ${
											this.limitTitle(secondPost.title?.rendered) ||
											"ETN mejora su EBITDA ajustado un 38,9% en 2024"
										}
                  </h3>
                  <p class="last-notes__card__caption-description">
                    ${
											this.limitExcerpt(secondPost.excerpt?.rendered) ||
