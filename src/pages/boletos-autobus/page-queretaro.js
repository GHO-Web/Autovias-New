...
class AppQueretaro extends HTMLElement {
	async connectedCallback() {
		...
		<app-modal-doters></app-modal-doters>
            <app-banner-slider
                slides-data='[
                {"id": "slide1", "title": "Banner 1", "image": "../src/assets/img/boletos-autobus/queretaro/banner/Queretaro_banner_web.webp","mediumImage": "./src/assets/img/boletos-autobus/queretaro/banner/tablet/Queretaro_tablet.webp", "smallImage": "./src/assets/img/boletos-autobus/queretaro/banner/mobile/Queretaro_mobile.webp", "link": "#index.html/banner1"}]'
            >
            </app-banner-slider>
	...
	async loadAndRenderGridItems() {
		...
		try {
			const response = await fetch(
				"../src/data/boletos-autobus/queretaro/lugares.json"
			);
			if (!response.ok) {
	...
	async loadAndRenderFoodCards() {
		...
		try {
			const response = await fetch(
				"../src/data/boletos-autobus/queretaro/platillos-ciudad-de-mexico.json"
			);
			if (!response.ok) {
				throw new Error(
	...
	async loadAndRenderDropdowns() {
		try {
			const response = await fetch(
				"../src/data/boletos-autobus/queretaro/dropdown-preguntas-frecuentes.json"
			);
			if (!response.ok) {
	...
	async _configureDestinationSlider() {
		...
		try {
			const response = await fetch(
				"../src/data/boletos-autobus/queretaro/card-opacity-destinations.json"
			);
			if (!response.ok) {
				throw new Error(
...
customElements.define("page-queretaro", AppQueretaro);
