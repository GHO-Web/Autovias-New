...
class AppIxtapa extends HTMLElement {
	async connectedCallback() {
		...
		<app-modal-doters></app-modal-doters>
            <app-banner-slider
                slides-data='[
                {"id": "slide1", "title": "Banner 1", "image": "../src/assets/img/global/banner/banner-monterrey.png","mediumImage": "../src/assets/img/global/banner/tablet/Ixtapa_tablet.webp", "smallImage": "../src/assets/img/global/banner/mobile/Ixtapa_mobile.webp", "link": "#index.html/banner1"}]'
            >
            </app-banner-slider>
	...
	async loadAndRenderGridItems() {
		...
		try {
			const response = await fetch(
				"../src/data/boletos-autobus/ixtapa/lugares.json"
			);
			if (!response.ok) {
	...
	async loadAndRenderFoodCards() {
		...
		try {
			const response = await fetch(
				"../src/data/boletos-autobus/ixtapa/platillos-ciudad-de-mexico.json"
			);
			if (!response.ok) {
				throw new Error(
	...
	async loadAndRenderDropdowns() {
		try {
			const response = await fetch(
				"../src/data/boletos-autobus/ixtapa/dropdown-preguntas-frecuentes.json"
			);
			if (!response.ok) {
	...
	async _configureDestinationSlider() {
		...
		try {
			const response = await fetch(
				"../src/data/boletos-autobus/ixtapa/card-opacity-destinations.json"
			);
			if (!response.ok) {
				throw new Error(
...
customElements.define("page-ixtapa", AppIxtapa);
