...
class AutobusAMonterrey extends HTMLElement {
    ...
    this.innerHTML = `
			...
			<section class="__section __section__monterrey__destinations">
             ...
             <section class="__section __section__grid" id="monterrey-grid-section">
              ...
              <section class="__section __section__monterrey__food">
                <p class="__paragraph__xl">Monterrey, capital industrial del norte de México, también es un referente gastronómico donde la tradición norteña se expresa con carácter y contundencia. Su cocina se distingue por el uso de carnes asadas, sabores intensos y técnicas heredadas del campo y la sierra. Desde parrilladas familiares hasta dulces artesanales, cada platillo refleja el orgullo regiomontano por su tierra y su identidad. Comer en Monterrey es disfrutar de una experiencia robusta, auténtica y profundamente ligada a la cultura del norte.</p>
			...
			<section class="cta">
				<div class="container">
					...
					<a href="../boletos-autobus/monterrey.html" class="btn btn--primary btn--large" target="_blank" rel="noopener noreferrer" aria-label="Comprar boletos" tabindex="0">Consulta horarios y compra tu boleto</a>
				</div>
			</section>
  ...
  async loadAndRenderGridItems() {
    ...
    try {
      const response = await fetch(
        "../../src/data/destinos/monterrey/lugares.json"
      );
      if (!response.ok) {
  ...
  async loadAndRenderFoodCards() {
    ...
    try {
      const response = await fetch(
        "../../src/data/destinos/monterrey/platillos.json"
      );
      if (!response.ok) {
        throw new Error(
  ...
  async loadAndRenderDropdowns() {
    try {
      const response = await fetch(
        "../../src/data/destinos/monterrey/dropdown-preguntas-frecuentes.json"
      );
      if (!response.ok) {
...
customElements.define("page-autobus-a-monterrey", AutobusAMonterrey);
