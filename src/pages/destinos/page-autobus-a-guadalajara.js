...
class AutobusAGuadalajara extends HTMLElement {
	async connectedCallback() {
		this.innerHTML = `
			...
			<section class="__section __section__monterrey__destinations">
				<app-section-title section-title="Viaja a Guadalajara en autobús con ETN Turistar Lujo"></app-section-title>
				<p class="__subtitle__destinos">Guadalajara</p>
				<p class="__paragraph">Guadalajara, cuna del mariachi y el tequila, es una ciudad vibrante donde la tradición y la modernidad se entrelazan con elegancia. Sus monumentos históricos, barrios llenos de arte y una gastronomía inigualable hacen de cada visita una experiencia única. ETN Turistar Lujo ofrece un servicio de autobús de lujo que conecta esta joya tapatía con diversos destinos, garantizando un viaje cómodo, seguro y sin estrés.</p>
			...
			<section class="cta">
				<div class="container">
					...
					<a href="../boletos-autobus/guadalajara.html" class="btn btn--primary btn--large" target="_blank" rel="noopener noreferrer" aria-label="Comprar boletos" tabindex="0">Consulta horarios y compra tu boleto</a>
				</div>
			</section>
  ...
  async loadAndRenderGridItems() {
    ...
    try {
      const response = await fetch(
        "../../src/data/destinos/guadalajara/lugares.json"
      );
      if (!response.ok) {
  ...
  async loadAndRenderFoodCards() {
    ...
    try {
      const response = await fetch(
        "../../src/data/destinos/guadalajara/platillos.json"
      );
      if (!response.ok) {
        throw new Error(
  ...
  async loadAndRenderDropdowns() {
    try {
      const response = await fetch(
        "../../src/data/destinos/guadalajara/dropdown-preguntas-frecuentes.json"
      );
      if (!response.ok) {
...
customElements.define("page-autobus-a-guadalajara", AutobusAGuadalajara);
