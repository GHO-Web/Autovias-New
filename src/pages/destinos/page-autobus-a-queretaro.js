...
<section class="__section __section__monterrey__destinations">
	<app-section-title section-title="Viaja a Querétaro en autobús con ETN Turistar Lujo"></app-section-title>
	<p class="__subtitle__destinos">Querétaro</p>
	<p class="__paragraph">
		Querétaro, Patrimonio Cultural de la Humanidad, combina historia, modernidad y una ubicación estratégica que la convierte en uno de los destinos más atractivos del centro de México. Con ETN Turistar Lujo, tu viaje a Querétaro será tan placentero como el destino mismo: disfruta de asientos reclinables, entretenimiento a bordo y un servicio de primera clase que te acompañará durante todo el trayecto.
	</p>
	<p class="__paragraph">
		Explora el encantador Centro Histórico de Querétaro, con sus casonas coloniales, templos barrocos y callejones llenos de vida. Admira el imponente Acueducto, símbolo de la ciudad, y visita museos como el Regional o la Casa de la Zacatecana. Si te gustan las escapadas naturales, aventúrate hacia la Peña de Bernal o las rutas vinícolas y queseras de Tequisquiapan y Ezequiel Montes. Querétaro te ofrece cultura, sabor y naturaleza en cada rincón.
	</p>
</section>
...
<section class="__section __section__monterrey__food">
	<p class="__paragraph__xl">
		La gastronomía queretana es un reflejo de su historia y de la mezcla de culturas que la enriquecen. Degusta el tradicional nopal en penca, las enchiladas queretanas, el chivo tapeado o el fiambre serrano. Acompaña tus platillos con un vino de la región o una refrescante cerveza artesanal local. En Querétaro, cada bocado es una invitación a saborear México con elegancia y tradición.
	</p>
...
<section class="cta">
	<div class="container">
		<h2 class="cta__title">¡Viaja a Querétaro con ETN Turistar Lujo!</h2>
		<p class="cta__subtitle">
			Vive una experiencia de viaje cómoda, segura y llena de estilo. Descubre por qué Querétaro es uno de los destinos más visitados del Bajío.
		</p>
		<a href="../boletos-autobus/queretaro.html" class="btn btn--primary btn--large" target="_blank" rel="noopener noreferrer" aria-label="Comprar boletos" tabindex="0">
...
<app-button-eva-trip></app-button-eva-trip>
  ...
  async loadAndRenderGridItems() {
    ...
    try {
      const response = await fetch(
        "../../src/data/destinos/queretaro/lugares.json"
      );
      if (!response.ok) {
  ...
  async loadAndRenderFoodCards() {
    ...
    try {
      const response = await fetch(
        "../../src/data/destinos/queretaro/platillos.json"
      );
      if (!response.ok) {
        throw new Error(
  ...
  async loadAndRenderDropdowns() {
    try {
      const response = await fetch(
        "../../src/data/destinos/queretaro/dropdown-preguntas-frecuentes.json"
      );
      if (!response.ok) {
...
customElements.define("page-autobus-a-queretaro", AutobusAQueretaro);
